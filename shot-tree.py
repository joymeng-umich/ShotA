...
def analyze_shot_tree(video_obj):
    def is_efficient(video_obj):
        ...
        # Efficient if hip is rising (decreasing y-coordinate)
        efficient = end_frame_hip_y <= prev_frame_hip_y
        print(f"\nQuestion: Is it efficient?")
        print(f"  Hip movement analysis: End frame Y={end_frame_hip_y}, Previous frame Y={prev_frame_hip_y}")
        print(f"  Decision: {'Efficient' if efficient else 'Not efficient'} (Hip is {'rising' if efficient else 'dropping'})")
        return efficient
    ...
    def is_shot_while_jumping(video_obj):
        ...
        print(f"  Analyzing hip deceleration before shot release. Deceleration: {deceleration}")

        # If deceleration is insignificant, it's more likely a jump shot
        if deceleration > -10:  # Threshold for significant deceleration
            print("  Decision: Shot while jumping (Insignificant deceleration)")
            return True
        else:
            print("  Decision: Pull Up Shot (Significant deceleration)")
            return False
    ...
    tree = (
        "Is it efficient?",
        (
            "Is it a shot while jumping?",
            ("1 motion shot", None, None),
            ("pull up shot", None, None)
        ),
        ("dropping shot", None, None)
    )
    ...
    while node:
        question = node[0]
        if question == "Is it efficient?":
            decision = is_efficient(video_obj)
        elif question == "Is it a shot while jumping?":
            decision = is_shot_while_jumping(video_obj)
        else:
            # Final decision
